...
{
	// Ako je broj 0 funkcija vraæa string "0"
	if (number == 0)
	return (char*)"0";
	// Bool varijabla koja oznaæava je li broj negativan (zadano je false)
	short is_negative = 0;
	if (number < 0) {
	...
	// Korištenjem matematike možemo izraèunati koliko broj ima znamenaka
	// Log10 nekog broja æe uvijek biti u skupu vrijednosti [brojZnamenki – 1,
	...
	// Varijabla output je pokazivaè kojem je alocirano memorije koliko broj ima
	...
	for (int i = size - 1; i >= 0; i--, number /= 10) {
		...
		// Broj poveæamo za '0' jer to oznaèava ASCII broj znamenke 0
		output[i + (is_negative ? 1 : 0)] = number % 10 + '0';
	}
	// Vraæanje pokazivaèa na znakovno polje
	return output;
}
